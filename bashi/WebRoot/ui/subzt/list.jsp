...
<body>
								...
								<tbody id="xx">
									<s:iterator value="recordList">
										<tr>
											...
											<th>${brief}</th>
											<th><a href="#">查看</a></th> 
											<th><a href="file/${img}" target="_blank">查看</a></th>
											<th>${url}</th>
											<th>${zhuanti.zbt}</th>
											<th>${father_address.name}</th>
											<th>${son_address.name}</th>
											<th>${time}</th>
											<td>
												<div
													...
													<button class="btn btn-xs btn-info" data-rel="tooltip"
														title="修改"
														onclick="bHref('${pageContext.request.contextPath}/ui/subzt_editUI.htm?id=${id}')">
														<i class="icon-edit bigger-120"></i>
													</button>
													<button class="btn btn-xs btn-danger del-confirm"
														data-rel="tooltip" title="删除"
														onclick="del('${pageContext.request.contextPath}/ui/subzt_delete.htm?ids=${id }')">
									...
									<tr>
										...
										<td colspan="13">
											<button class="btn btn-xs btn-success" data-rel="tooltip"
												title="增加一行"
												onclick="bHref('${pageContext.request.contextPath}/ui/subzt_addUI.htm')">
												<i class="icon-plus-sign bigger-150"></i>
											</button>
											<button id="bootbox-confirm" class="btn btn-xs btn-danger"
												data-rel="tooltip" title="删除选中行"
												onclick="del('${pageContext.request.contextPath}/ui/subzt_delete.htm?ids='+checkedValue('ids'))">
	...
	<script type="text/javascript">
		function gotoPage(pageNum) {
			window.location.href = "subzt_list.action?pageNum=" + pageNum;
		}
		$(function() {
